...
class_name BossRavanPrototype3D

## Boss: Inquisidor Ravan, Lâmina da Chama Branca
## Placeholder boss for the vertical slice

@export var max_health: float = 300.0
@export var patrol_radius: float = 1.5
...
@export var loot_drop_chance: float = 1.0
...
# Phase 2 check
	...
	_time += delta
	var offset := Vector3(cos(_time * 0.5) * patrol_radius, 0.0, sin(_time * 0.5) * patrol_radius)
...
func receive_damage(amount: float, source: Node = null) -> void:
	...
	if health_component.has_method("take_damage"):
		health_component.call("take_damage", amount, source)
...
func _on_damaged(amount: float, source: Node) -> void:
	print("Boss Ravan received %.1f damage. Health: %.1f/%.1f" % [
		amount,
...
func _flash_damage() -> void:
	...
	flash_material.albedo_color = Color(1.0, 0.3, 0.1, 1.0)
...
func _attack_target(target: Node3D) -> void:
	...
	if target.has_method("receive_damage"):
		...
		if health_component != null and health_component.has_method("take_damage"):
			health_component.call("take_damage", damage, self)
...
func _move_toward_position(target_position: Vector3, delta: float) -> void:
	...
	var speed := patrol_speed * 1.5 if _phase_2 else patrol_speed
...
func _apply_horizontal_velocity(target_velocity: Vector3, delta: float) -> void:
	velocity.x = move_toward(velocity.x, target_velocity.x, patrol_speed * 6.0 * delta)
	velocity.z = move_toward(velocity.z, target_velocity.z, patrol_speed * 6.0 * delta)
